...
def incremental_train_and_eval(args, base_lamda, adapt_lamda, u_t, label2id, uncertainty_distillation, 
    ...
    def compute_separation_loss(ema_centers, old_cn, nb_cl, delta=0.5):

        if old_cn == 0 or nb_cl == 0:
            return torch.tensor(0.0, device=ema_centers.device)
        
        ema_centers_norm = F.normalize(ema_centers, p=2, dim=1)

        old_centers = ema_centers_norm[:old_cn]
        new_centers = ema_centers_norm[old_cn:]
        
        cosine_sim_matrix = torch.matmul(old_centers, new_centers.t())

        hinge_loss = torch.relu(delta - cosine_sim_matrix)

        separation_loss = hinge_loss.mean()
        ...
        for batch_idx, (indexs, inputs, inputs_s, targets, flags, on_flags) in enumerate(trainloader):
            ...
            if include_unlabel and epoch >= warmup_epochs:                           
                ...
                if iteration > start_iteration and use_ulb_aug and epoch != 0:
                    ...
                    q_pseudo_label = torch.softmax(q_cosine_scores[:, old_cn:total_cn], dim=1)
                    q_predict_class = q_pseudo_label.max(1)[1] + old_cn
                    
                    q_s_cosine_scores = F.linear(feats_s_ulb, F.normalize(prototypes, p=2, dim=1)) / 0.1
                    q_s_pseudo_label = torch.softmax(q_s_cosine_scores[:, old_cn:total_cn], dim=1)
                    q_s_predict_class = q_s_pseudo_label.max(1)[1] + old_cn

                    consistency_mask = q_predict_class.eq(q_s_predict_class).float()
                    if not mask.bool().all():
                        ...
                        if ulb_aug_old_mask.any():
                            ulb_aug_old_acc = q_predict_class[ulb_aug_old_mask].eq(gt[ulb_aug_old_mask]).float().mean().item()
                            ulb_old_acc = predicted_classes[ulb_aug_old_mask].eq(gt[ulb_aug_old_mask]).float().mean().item()
                        else:
                            ulb_aug_old_acc = torch.tensor(0.0).to(device)
                            ulb_old_acc = torch.tensor(0.0).to(device)
                    else:
                        ulb_aug_new_acc = torch.tensor(-1.0).to(device)
                        ulb_new_acc = torch.tensor(-1.0).to(device)


                    consloss_ulb_aug = ce_loss(outputs_s_ulb, q_predict_class, True, reduction='none') * torch.logical_not(mask.bool()).float() * valid_mask * consistency_mask
                                        
                    consloss_ulb_aug = consloss_ulb_aug.mean()
                    sep_loss = compute_separation_loss(prototypes, old_cn, nb_cl, delta=delta_sep)
                else:
                    consloss_ulb_aug = torch.tensor(0.0).to(device)
                    sep_loss = torch.tensor(0.0).to(device)
                
                loss = lambda_ce * suploss_lb + lambda_kd * (suploss_kd + suploss_kd_ulb) + lambda_con * (conloss_lb + conloss_ulb) + lambda_cons * (consloss_ulb + consloss_ulb_aug) + lambda_sep * sep_loss
            else:

...
from utils.sa_mixup import RandAugment
from PIL import Image, ImageEnhance, ImageOps


class Keep_Autoaugment_Low(object):
    def __init__(self, mean, std, length, N, M, early=False, train_transform=None):
...
class Keep_Autoaugment(object):
    def __init__(self, mean, std, length, N, M, num, batch_size=256, early=False, train_transform=None, device=None):
        if train_transform == None:
            normalize = transforms.Normalize(mean, std)
            train_transform = transforms.Compose([
            RandAugment(3, 5), 
        ...
        self.device = device
    def __call__(self, images, model, size=(224, 224)):
        results = []
        toTensors = transforms.ToTensor()
        for i in range(self.num):
            img = Image.open(random.sample(images, 1)[0]).convert('RGB').resize(size)
            results.append(toTensors(img))
        ...
        for i in range(0, len(images), self.batch_size):
            ...
            batch_images = batch_images.to(self.device).clone().detach()
            batch_images.requires_grad = True

            if self.early:
                preds, _ = model.classifier(batch_images, True)['sim']
            else:
                preds = model(batch_images)
            
            scores, _ = torch.max(preds, 1)
            scores.mean().backward()
            grads = torch.abs(batch_images.grad).max(dim=1)[0]

            for img, grad in zip(batch_images, grads):
                slc_ = grad.view(-1)
                slc_ -= slc_.min()
                slc_ /= slc_.max()
                slc_ = slc_.view(1, img.size(1), img.size(2))

                mask_zero = torch.zeros_like(img)
                while True:
                    y = np.random.randint(img.size(1))
                    x = np.random.randint(img.size(2))
                    y1 = np.clip(y - self.length // 2, 0, img.size(1))
                    y2 = np.clip(y + self.length // 2, 0, img.size(1))
                    x1 = np.clip(x - self.length // 2, 0, img.size(2))
                    x2 = np.clip(x + self.length // 2, 0, img.size(2))

                    if slc_[0, y1:y2, x1:x2].max() >= 0.6:
                        img = self.denomal(img)
                        break

                img = self.pil(img.cpu())
                img = self.auto(img)
                img = self.tensor(img)
                img[:, y1:y2, x1:x2] = mask_zero[:, y1:y2, x1:x2]
                img = self.trans(self.pil(img))
        ...
        return torch.stack(processed_images).cpu()
